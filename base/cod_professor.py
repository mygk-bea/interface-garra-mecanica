...
PORTA = "COM8" # altere para sua porta
BAUDRATE = 9600
...
def comando_motor (motor_num):
    ...
    if not passos: passos = '0'
    if not delay: delay = '10'
...
def erro_angulos (thetas, x_desejado, y_desejado, z_desejado):
    ...
    # Adiciona restrição extra para fechar sistema
    #Ex.: theta2 + theta3 + theta4 = 0
    return [
        x_atual - x_desejado,
        y_atual - y_desejado,
        z_atual - z_desejado,
...
def inversa_fsolve(x, y, z, chute_inicial=None):
    global thetal_atual, theta2_atual, theta3_atual, theta4_atual
    if chute_inicial is None:
        chute_inicial = [thetal_atual, theta2_atual, theta3_atual, theta4_atual]

    sol = fsolve(erro_angulos, chute_inicial, args = (x, y, z))
    return sol # retorna [t1, t2, t3, t4]

#
# Movimento incremental (mantido igual)
#
...
def mover_para_coordenada_seguro():
    ...
    xs, ys, zs = direta (thetal_atual, theta2_atual, theta3_atual, theta4_atual)
    ...
    for _ in range (n_passos):
        J = calcular_jacobiano (thetal_atual, theta2_atual, theta3_atual, theta4_atual)
...
def mover_para_absoluto_fsolve():
    ...
    deltas = [delta_theta(t1, thetal_atual),
    ...
    thetal_atual, theta2_atual, theta3_atual, theta4_atual = t1,t2,t3, t4
    atualizar_plot()
...
def atualizar_plot():
    ...
    for i in range(4): ax.plot3D ([xx[i], xx[i]], [yy[i], yy[i]], [0, base_height], color='gray')
    ...
    ax.set_title('Braço Robótico 4R'); ax.view_init(elev = 30, azim=60)
    ax.set_box_aspect ([1,1,0.5])
    canvas.draw()

# Inicializa posição
xs_init, ys_init, zs_init = direta (thetal_atual, theta2_atual, theta3_atual, theta4_atual)
...
for i in range (4):
    ...
    e_passos = tk. Entry (frame, width = 10); e_passos.pack(anchor='w'); passos_entry.append(e_passos)
